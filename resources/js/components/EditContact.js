...
const EditContact = (props) => {
    ...
    const[seletedItem,setSelectedItem] = useState([]); // after chnage then selected

    const[isSelect, setIsSelected] = useState([]);  // when get selected item
    ...
    const fetchData = () => {
        axios.get(`/contact/${id}/edit`)
          .then((res) => {

            var selectArray = JSON.parse(res.data.contact.fruits);
            ...
            setData({
                ...
                bio: res.data.contact.bio,
                birthday: res.data.contact.dob,
                gender: res.data.contact.gender
            });
            setIsRadio({
                [res.data.contact.gender]: true,
            });
            
            let checkedArray = JSON.parse(res.data.contact.sports);
    ...
    const updateContact = async (e) => {
        e.preventDefault();
        const res = await axios.patch(`/contact/${id}`, [data,isChecked, seletedItem]);
    ...
    var filtered  = Item.filter(function(item) {
        return isSelect.indexOf(item.id) !== -1;
    });
